...
process.env.OSV_API_KEY = 'test-osv-api-key';

// Mock external services
jest.mock('@octokit/rest', () => ({
  Octokit: jest.fn().mockImplementation(() => ({
    repos: {
      get: jest.fn().mockImplementation(async ({ owner, repo }: { owner: string; repo: string }) => ({
...
// Mock database
const mockRepos = new Map<string, Repository>();
...
jest.mock('../server/db/repositories', () => {
  const mockModel = {
    ...
    deleteMany: jest.fn().mockImplementation(async () => {
      mockRepos.clear();
      return { deletedCount: mockRepos.size };
    })
  };

  return { RepoModel: mockModel, RepoModelMongoose: mockModel };
});
...
jest.mock('../server/middleware/auth', () => ({
  validateAuthToken: jest.fn().mockImplementation((req, res, next) => {
